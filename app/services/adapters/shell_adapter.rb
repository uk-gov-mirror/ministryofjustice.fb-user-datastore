...
class Adapters::ShellAdapter
  ...
  def self.exec(*args)
    ...
    raise CmdFailedError.new(cause: "#{$?}", message: I18n.t('error_messages.cmd_failed', name: cmd_line)) unless result
  end
  ...
  def self.capture_with_stdin(cmd: [], stdin: nil)
    ...
    unless status.success?
      raise CmdFailedError.new(cause: "#{$?}", message: I18n.t('error_messages.cmd_failed', name: cmd_line))
    end
    stdout_str
  end
